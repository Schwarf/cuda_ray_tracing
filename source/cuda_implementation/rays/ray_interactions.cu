//
// Created by andreas on 08.01.22.
//
...
__device__ __host__ void specular_scatter(const IRay &incoming_ray, const IHitRecord &incoming_hit_record, IRay & scattered_ray)
{
	...
	auto reflected_direction = reflection(incoming_ray.direction_normalized(), incoming_hit_record.hit_normal());
//	printf( "Specular Reflected direction  %f, %f, %f \n", reflected_direction[0], reflected_direction[1], reflected_direction[2]);
	scattered_ray.set_origin(incoming_hit_record.hit_point());
	scattered_ray.set_direction(reflected_direction);
//	printf( "Specular Reflected hit  %f, %f, %f \n", incoming_hit_record.hit_normal()[0], incoming_hit_record.hit_normal()[1], incoming_hit_record.hit_normal()[2]);
